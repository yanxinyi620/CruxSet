...
Cloudflare 云端对非管理员账户限制：最多保留 10 张实验图片、20 个分割任务、北京时间每日创建 20 个分割任务、同时公开 10 面墙。每个账户（包括管理员）最多同时排队或运行 2 个任务的原有规则不变。管理员不受新增数量额度限制。

删除图片会清理其关联任务与校准；删除任务会释放任务保留额度，已保存校准的独立副本继续保留。每日计数在任务创建成功时增加（包括之后触发失败的任务），拒绝创建不计数，删除任务或图片不退回当天次数，北京时间零点开始新一天。

实验台显示已用额度，并提供“管理我的墙面”入口，在新标签页打开 `/me?panel=my-walls`。创作者通过“我的 → 我的墙面”删除自己的公开墙面；删除同时移除全部关联线路、岩点和发布图片，释放墙面额度。来源实验和校准不会因此删除。撤销实验台授权后仍可删除自己已有墙面。存储清理暂时失败时会保留清理记录，由定时任务重试。已删除墙面的旧发布回执不会恢复墙面，需要保存新校准再发布。

部署前应用 `edge/migrations/0011_lab_quotas.sql`，再部署 Worker 和 Web。迁移保留已有超额内容，并从现存任务（含软删除记录）回填每日用量。本地实验台不应用这组额度。
