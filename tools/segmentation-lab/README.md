...
通过 <http://localhost:5173/segmentation-lab/> 操作。8765 的 API 只接受 FastAPI 签名请求。手动启动时不要复制仓库中的示例字符串作为密钥；开发启动脚本会生成并保存私有随机密钥。
...
2. 在 **02 选择模型** 选择 `sam2`，从“基线（推荐）”预设开始运行。基线参数为每边64点、批量8、质量阈值0.83、稳定度0.92、裁剪层0；“少漏检”使用质量阈值0.75、稳定度0.90，“少误检”使用质量阈值0.85、稳定度0.95，其余参数相同。任务列表支持查看已完成结果。
3. 在 **03 分割结果** 打开结果，确认候选数量与轮廓覆盖范围。
4. 在 **04 人工校准** 加载结果：大体积显示在底层，小岩点显示在上层，新增模式可直接在已有体积的 polygon 上绘制岩点。
5. 保存校准结果，并从首页导出 SVG；需要公开使用时，显式选择发布目标。
6. 通过 **04 人工校准**中校准按钮右侧的“管理我的墙面”，在新标签页打开对应主站。本地与云端创作者均可管理、删除自己的墙面及关联线路和发布图片；不会因此删除来源实验和校准。
...
分割结果保存全图坐标的 polygon、bbox、像素面积及按位压缩的 bbox 局部 mask。像素 IoU 去重只解码 bbox 重叠区域；服务层使用 `IoU >= 0.90` 规则。

逐候选 PNG 与输入尺寸相同，写出时逐张恢复并释放。推理和输出数组会占用内存，实际用量随输入尺寸与参数变化。
...
采样网格覆盖模型输入，输出映射回原图坐标，内部裁剪层固定为 0。
每次运行创建独立任务，结果记录对应的模型和采样参数。
