...
try{const source=readFileSync(join(root,'cloudfunctions/getLayoutImageUrl/index.js'),'utf8');execFileSync(process.execPath,['--check',join(root,'cloudfunctions/getLayoutImageUrl/index.js')],{stdio:'pipe'});const pkg=JSON.parse(readFileSync(join(root,'cloudfunctions/getLayoutImageUrl/package.json'),'utf8'));if(!pkg.dependencies?.['wx-server-sdk'])throw new Error('wx-server-sdk is not declared');if(!source.includes('getTempFileURL'))throw new Error('temporary image URL generation is missing')}catch(error){console.error(`FAIL: cloudfunction getLayoutImageUrl is not deployable: ${error.message}`);process.exitCode=1}
