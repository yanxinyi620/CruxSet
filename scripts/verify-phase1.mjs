...
for(const fn of ['login','saveProblem','deleteProblem','adminWall']){try{const source=readFileSync(join(root,'cloudfunctions',fn,'index.js'),'utf8');execFileSync(process.execPath,['--check',join(root,'cloudfunctions',fn,'index.js')],{stdio:'pipe'});const pkg=JSON.parse(readFileSync(join(root,'cloudfunctions',fn,'package.json'),'utf8'));if(!pkg.dependencies?.['wx-server-sdk'])throw new Error('wx-server-sdk is not declared');if(fn==='adminWall'&&(!source.includes("'publishWall'")||!source.includes("'updateWallHolds'")||!source.includes('WALL_LOCKED')))throw new Error('flat wall publishing flow is incomplete')}catch(error){console.error(`FAIL: cloudfunction ${fn} is not deployable: ${error.message}`);process.exitCode=1}}
