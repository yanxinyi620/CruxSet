...
const collectJs=(directory)=>readdirSync(directory,{withFileTypes:true}).flatMap(entry=>entry.isDirectory()?entry.name==='node_modules'?[]:collectJs(join(directory,entry.name)):entry.name.endsWith('.js')?[join(directory,entry.name)]:[])
const collectFiles=(directory)=>readdirSync(directory,{withFileTypes:true}).flatMap(entry=>entry.isDirectory()?['node_modules','.venv','.git','dist','dist-local'].includes(entry.name)?[]:collectFiles(join(directory,entry.name)):[join(directory,entry.name)])
