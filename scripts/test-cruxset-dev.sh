...
assert_eq '' "$PUBLISH_KEY" key-not-loaded-until-start
...
assert_eq https://storage.example/upload "$CRUXSET_CLOUDBASE_STORAGE_URL" env-file-storage-url
assert_eq https://function.example/publish "$CRUXSET_CLOUDBASE_FUNCTION_URL" env-file-function-url
rm -f "$env_file"
