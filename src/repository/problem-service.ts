...
export interface SaveProblemInput { wall: Wall; layout: Layout; draft: Omit<ProblemDraft, 'id'|'number'> }
...
export class MemoryProblemService {
  ...
  async save(input: SaveProblemInput): Promise<Problem> { const number = createProblemNumber(this.counter); const problem = createProblem({ ...input.draft, id: createId('problem'), number }, input.wall, input.layout); await this.repository.create(problem); this.counter += 1; return problem }
