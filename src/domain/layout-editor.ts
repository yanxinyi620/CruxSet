...
export class LayoutEditor {
  ...
  add(input: { x: number; y: number; radius?: number; kind?: HoldKind }): Hold { this.checkpoint(); const hold: Hold = { id: `H${String(this.holds.length + 1).padStart(3, '0')}`, x: Math.max(0, Math.min(1, input.x)), y: Math.max(0, Math.min(1, input.y)), radius: input.radius ?? .018, kind: input.kind ?? 'hold' }; this.holds.push(hold); return { ...hold } }
