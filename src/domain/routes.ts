import type { FootRule,Grade,HoldRole,Layout,Problem,ProblemHolds,Wall } from './types.js'
...
export interface ProblemDraft { id:string; number:string; wallId:string; layoutId:string; name?:string; description?:string; angle:number; grade:string; footRule?:string; holds:Partial<ProblemHolds>; createdBy:string; now?:number }
export function createProblem(draft:ProblemDraft,wall:Wall,layout:Layout):Problem {
  if(draft.wallId!==wall.id||draft.layoutId!==layout.id||layout.wallId!==wall.id) throw new Error('wall or layout mismatch')
