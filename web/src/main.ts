...
const render = async () => {
  ...
  const create = panel==='layout-choice'?chooser:panel==='new-wall'?`${back}<h1>新建墙面</h1><p class="lead">上传墙图后会创建私有墙面和一个可标注的草稿 Layout。</p><div class="field"><label>墙面名称</label><input id="wall-name" placeholder="如：日坛 Spraywall"></div><div class="field"><label>Layout 名称</label><input id="layout-name" value="首次标注"></div><div class="field"><label>墙面图片</label><input id="wall-image" type="file" accept="image/jpeg,image/png,image/webp"></div><div id="wall-create-error" class="editor-toast" style="display:none"></div><button class="hero-card" data-create-wall><span>本地保存</span><b>创建草稿墙面</b><em>草稿不会公开，也不能定线。</em></button>`:panel==='drafts'?`${back}<h1>我的草稿</h1><p class="lead">仅在这里继续标注未发布 Layout。</p>${drafts.flatMap(({wall,layouts})=>layouts.map(layout=>`<article class="mine-card">${thumb}<span><b>${wall.name}</b><em>${layout.name} · 草稿 · 可继续标注</em><small>继续标注</small></span><mark>私有</mark></article>`)).join('')||'<p class="note">没有未发布草稿</p>'}`:`<h1>创建</h1><p class="lead">从这里新建内容，或继续完成尚未发布的墙面标注。</p><button class="hero-card" data-panel="new-wall"><span>从真实墙面开始</span><b>新建墙面</b><em>上传照片后进入首次标注；未发布前保持私有。</em></button><h3>继续创建</h3><button class="action-card" data-panel="drafts"><i>✦</i><span><b>我的草稿</b><em>${drafts.length} 面墙含未发布 Layout，仅此处可继续标注</em></span><strong>›</strong></button><button class="action-card" data-choose="create"><i>＋</i><span><b>新建线路</b><em>仅能选择已发布 Layout</em></span><strong>›</strong></button><p class="lock"><b>发布即公开并锁定：</b>发布后可浏览、可定线；草稿不能定线。</p>`
