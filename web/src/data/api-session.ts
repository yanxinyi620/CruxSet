...
export class ApiSession extends PreviewSession {
  ...
  async refresh() { const data = await this.api.loadBootstrap(); this.walls = structuredClone(data.walls.map(normalizeWall)); this.problems = structuredClone(data.problems) as Problem[]; this.currentUserId = data.user?.id ?? null }
  ...
  override async createWall(data: CreateWallInput) { if (!data.image) throw new Error('WALL_IMAGE_REQUIRED'); const wall = await this.api.createWall({ name: data.name, image: data.image, imageWidth: data.imageWidth, imageHeight: data.imageHeight }); this.replaceWall(wall); return wall }
  override async updateWallHolds(id: string, holds: Hold[]) { const result = await this.api.saveWallHolds(id, holds); this.replaceWall(result.wall); return this.getWall(id) }
  override async publishWall(id: string, holds: Hold[]) { const saved = await this.api.saveWallHolds(id, holds); this.replaceWall(saved.wall); try { const published = await this.api.publishWall(id); this.replaceWall(published.wall) } catch (error) { throw error } return this.getWall(id) }
  ...
  override async deleteProblem(id: string) { await this.api.deleteProblem(id); this.removeProblem(id); return { ok: true } }
  ...
  override async deleteWall(id: string): Promise<{ ok: true }> { await this.api.deleteWall(id); this.removeWall(id); return { ok: true as const } }
}
