...
export type WebCapabilities = { readOnly: boolean; writes: boolean; authentication: boolean; wallAuthoring: boolean; imageUpload: boolean; aiJobs: boolean; segmentationLab?: boolean; manageLabAccess?: boolean; manageOwnWalls?: boolean }
...
export class LocalApiClient {
  ...
  async updateLabAccess(userId: string, enabled: boolean): Promise<Pick<AdminUser, 'id' | 'labEnabled'>> { const result = await this.request(`/api/v1/auth/admin/users/${encodeURIComponent(userId)}/lab-access`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ enabled }) }); return result.user as Pick<AdminUser, 'id' | 'labEnabled'> }
  ...
  async loadBootstrap(): Promise<BootstrapData> { return (await this.get('/api/v1/bootstrap')) as unknown as BootstrapData }
  async loadAdminWalls(): Promise<Wall[]> { const result = await this.get('/api/v1/walls'); return result.walls as Wall[] }
  ...
  async syncRoutes(wallId: string, action: 'preview' | 'sync'): Promise<import('./route-sync.js').SyncResult> { return this.request(`/api/v1/admin/walls/${encodeURIComponent(wallId)}/route-sync`, {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,target:'cloudbase'})}) as unknown as Promise<import('./route-sync.js').SyncResult> }
  private get(path: string) { return this.request(path) }
  private async request(path: string, init: RequestInit = {}): Promise<Record<string, unknown>> { const fetcher = this.fetcher, startedAt = performance.now(); try { const response = await fetcher(`${this.baseUrl}${path}`, { credentials: 'include', ...init }); if (!response.ok) { const body = await response.json().catch(() => null) as { error?: { code?: string; message?: string } } | null; throw new ApiError(body?.error?.message || '服务暂时不可用，请稍后重试', body?.error?.code) } return await response.json() as Record<string, unknown> } finally { if ((import.meta as ImportMeta & { env?: { DEV?: boolean } }).env?.DEV) console.info('[api]', init.method || 'GET', path, `${Math.round(performance.now() - startedAt)}ms`) } }
}
