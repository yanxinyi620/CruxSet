"""One-time offline migration from legacy wall documents to Wall-only data.

This module is intentionally outside the live repository and API contracts.
"""
...
_LAYOUT_FIELDS = (
    ...
    "geometryType", "holds", "published", "createdAt", "updatedAt",
)
...
def flatten_legacy_documents(
    ...
    """Create independent Wall documents and repoint legacy problems to them."""
...
def migrate_sqlite_wall_only(database: str | Path, backup: str | Path) -> None:
    """Back up and transactionally flatten a legacy SQLite document store."""
    ...
    try:
        # Acquire the writer lock before backing up, so WAL changes and the
        # subsequently transformed input belong to one committed snapshot.
