...
export interface Env { ASSETS: Fetcher; DB?: D1Database; MEDIA?: R2Bucket; PASSWORD_PEPPER?: string }
...
async function derive(password: string, salt: Uint8Array, iterations: number, pepper: string) { const key=await crypto.subtle.importKey('raw',new TextEncoder().encode(password+pepper),'PBKDF2',false,['deriveBits']); const bits=await crypto.subtle.deriveBits({name:'PBKDF2',salt,iterations,hash:'SHA-256'},key,PBKDF2_KEY_BYTES*8); return new Uint8Array(bits) }
async function hashPassword(password: string, pepper: string) { const salt=crypto.getRandomValues(new Uint8Array(PBKDF2_SALT_BYTES)); return `pbkdf2-sha256$v1$${PBKDF2_ITERATIONS}$${b64(salt)}$${b64(await derive(password,salt,PBKDF2_ITERATIONS,pepper))}` }
async function passwordOk(encoded: string, password: string, pepper: string) { const p=encoded.split('$'); if(p.length!==5 || p[0]!=='pbkdf2-sha256' || p[1]!=='v1') return false; const iterations=Number(p[2]); if(!Number.isInteger(iterations)||iterations<100000) return false; const salt=Uint8Array.from(atob(p[3]),c=>c.charCodeAt(0)), expected=Uint8Array.from(atob(p[4]),c=>c.charCodeAt(0)), actual=await derive(password,salt,iterations,pepper); let diff=actual.length^expected.length; for(let i=0;i<actual.length;i++) diff|=actual[i]^(expected[i]??0); return diff===0 }
...
async function register(request: Request, db: D1Database, env: Env) {
  ...
  const id = `usr_web_${crypto.randomUUID()}`, now = Date.now(), passwordHash = await hashPassword(password, env.PASSWORD_PEPPER ?? ''), token = crypto.randomUUID() + crypto.randomUUID()
  await db.batch([
