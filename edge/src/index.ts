...
const worker: ExportedHandler<Env> = { async fetch(request, env) {
  ...
  if (pathname.startsWith('/api/v1/')) {
    ...
    if (pathname === '/api/v1/auth/admin/login' && request.method === 'POST') { try { const body=await request.json() as {email?:string,password?:string}; const email=String(body.email??'').trim().toLowerCase(), key=await rateKey(request,email); if(!(await rateAllowed(env.DB,key))) return error(request,'RATE_LIMITED','Too many login attempts',429); const row=await env.DB.prepare('SELECT u.id,u.display_name,a.role,a.email_normalized,a.password_hash FROM admins a JOIN users u ON u.id=a.user_id WHERE a.email_normalized=?').bind(email).first() as Record<string,unknown>|null; if(!row || !(await passwordOk(String(row.password_hash),String(body.password??'')))) { await rateFailed(env.DB,key); return json(request,{error:{code:'AUTH_REQUIRED',message:'Authentication required'}},{status:401}); } await rateSuccess(env.DB,key); const token=crypto.randomUUID()+crypto.randomUUID(); await env.DB.prepare('INSERT INTO sessions VALUES (?,?,?,?)').bind(await digest(token),row.id,Date.now()+28800000,Date.now()).run(); return json(request,{user:{id:row.id,email:row.email_normalized,displayName:displayName(row),isAdmin:row.role==='admin'}},{headers:{'Set-Cookie':`${cookie}=${token}; Max-Age=28800; Path=/; Secure; HttpOnly; SameSite=Lax`}}) } catch (error) { console.error('admin_login_failed', error instanceof Error ? error.stack : String(error)); return json(request,{error:{code:'LOGIN_FAILED',message:'Login service failed'}},{status:500}) } }
