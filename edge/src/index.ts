...
import type { LabEnv } from './lab/common.js'
import { listAllPublicWalls, listAllWallsForAdmin, listProblems, listWalls } from './browse.js'

export interface Env extends LabEnv { ASSETS: Fetcher; DB?: D1Database; MEDIA?: R2Bucket; SEGMENTATION_PUBLISH_KEY?: string }
...
async function rateKey(request: Request, email: string) { return `${request.headers.get('CF-Connecting-IP') ?? request.headers.get('X-Forwarded-For') ?? 'unknown'}|${email}` }
async function rateAllowed(db: D1Database, key: string) { const now=Date.now(), row=await db.prepare('SELECT failures,window_start,blocked_until FROM login_attempts WHERE key=?').bind(key).first() as Record<string,unknown>|null; if(!row) return true; if(Number(row.blocked_until)>now) return false; if(now-Number(row.window_start)>600000){ await db.prepare('DELETE FROM login_attempts WHERE key=?').bind(key).run(); return true } return Number(row.failures)<5 }
async function rateFailed(db: D1Database, key: string) { const now=Date.now(), row=await db.prepare('SELECT failures,window_start FROM login_attempts WHERE key=?').bind(key).first() as Record<string,unknown>|null; const failures=row && now-Number(row.window_start)<=600000 ? Number(row.failures)+1 : 1; await db.prepare('INSERT OR REPLACE INTO login_attempts (key,failures,window_start,blocked_until) VALUES (?,?,?,?)').bind(key,failures,row && now-Number(row.window_start)<=600000?Number(row.window_start):now,failures>=5?now+600000:0).run() }
async function rateSuccess(db: D1Database, key: string) { await db.prepare('DELETE FROM login_attempts WHERE key=?').bind(key).run() }
async function session(request: Request, db: D1Database) { const value = request.headers.get('Cookie')?.match(new RegExp(`${cookie}=([^;]+)`))?.[1]; if (!value) return null; return await db.prepare('SELECT u.id,u.display_name,a.email_normalized,a.role,a.lab_enabled,s.token_hash FROM sessions s JOIN users u ON u.id=s.user_id JOIN admins a ON a.user_id=u.id WHERE s.token_hash=? AND s.expires_at>?').bind(await digest(value), Date.now()).first() as Record<string, unknown> | null }
...
const worker: ExportedHandler<Env> = { async fetch(request, env) {
  ...
  if (pathname.startsWith('/api/v1/')) {
    ...
    const syncMatch = pathname.match(/^\/api\/v1\/admin\/walls\/([^/]+)\/route-sync$/)
    if (syncMatch && request.method === 'POST') {
      const user=await session(request,env.DB)
      if(user?.role!=='admin') return error(request,'FORBIDDEN','Administrator access required',403)
      try {
        const body=await request.json() as {action?:string;target?:string}
        if(!['preview','sync'].includes(body.action??'preview')||(body.target&&body.target!=='cloudbase')) return error(request,'INVALID_INPUT','Invalid sync action',422)
        const result=await syncWall(env.DB,decodeURIComponent(syncMatch[1]),String(user.id),(body.action??'preview') as 'preview'|'sync',cloudbaseBridge(env))
        return json(request,result,{headers:{'Cache-Control':'no-store'}})
      } catch(e) { return error(request,(e as any).code??'SYNC_FAILED',(e as Error).message,(e as any).status??409) }
    }
    ...
    if (pathname === '/api/v1/auth/admin/login' && request.method === 'POST') { try { const body=await request.json() as {email?:string,password?:string}; const email=String(body.email??'').trim().toLowerCase(), key=await rateKey(request,email); if(!(await rateAllowed(env.DB,key))) return error(request,'RATE_LIMITED','Too many login attempts',429); const row=await env.DB.prepare('SELECT u.id,u.display_name,a.role,a.lab_enabled,a.email_normalized,a.password_hash FROM admins a JOIN users u ON u.id=a.user_id WHERE a.email_normalized=?').bind(email).first() as Record<string,unknown>|null; if(!row || !(await passwordOk(String(row.password_hash),String(body.password??'')))) { await rateFailed(env.DB,key); return json(request,{error:{code:'AUTH_REQUIRED',message:'Authentication required'}},{status:401}); } await rateSuccess(env.DB,key); const token=crypto.randomUUID()+crypto.randomUUID(); await env.DB.prepare('INSERT INTO sessions VALUES (?,?,?,?)').bind(await digest(token),row.id,Date.now()+28800000,Date.now()).run(); return json(request,{user:{id:row.id,email:row.email_normalized,displayName:displayName(row),isAdmin:row.role==='admin',labEnabled:row.lab_enabled===1}},{headers:{'Set-Cookie':`${cookie}=${token}; Max-Age=28800; Path=/; Secure; HttpOnly; SameSite=Lax`}}) } catch (error) { console.error('admin_login_failed', error instanceof Error ? error.stack : String(error)); return json(request,{error:{code:'LOGIN_FAILED',message:'Login service failed'}},{status:500}) } }
    const mediaMatch = pathname.match(/^\/api\/v1\/media\/([^/]+)$/)
    if (pathname === '/api/v1/media/images' && request.method === 'POST') { const u=await session(request,env.DB); return u ? uploadMedia(request,env,u) : error(request,'FORBIDDEN','Administrator access required',403) }
    if (pathname === '/api/v1/admin/segmentation-walls' && request.method === 'POST') return segmentationPublish(request,env)
  ...
  if (/^\/segmentation-lab\/results\/[\w-]+\/?$/.test(pathname) || /^\/segmentation-lab\/calibrations\/?$/.test(pathname)) {
    const assetUrl=new URL(request.url);assetUrl.pathname=pathname.includes('/results/')?'/segmentation-lab/results':'/segmentation-lab/calibration'
