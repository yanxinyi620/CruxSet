...
async function createProblem(request: Request, db: D1Database, user: Record<string, unknown>) {
  try {
    ...
    if (!wall || wall.visibility !== 'public' || Number(wall.published) !== 1) return apiError('WALL_NOT_ROUTABLE', 'Wall is not published', 409)
    ...
    if (!angles.includes(angle) || !/^V(?:[0-9]|1[0-2])$/.test(grade) || !['feet_follow', 'specified', 'all'].includes(footRule)) return apiError('INVALID_INPUT', 'Invalid route settings', 400)
    ...
    if (!assignments.some((item) => item.role === 'start') || !assignments.some((item) => item.role === 'finish') || uniqueIds.length !== assignments.length) return apiError('INVALID_INPUT', 'Start and finish holds are required', 400)
    const placeholders = uniqueIds.map(() => '?').join(',')
    const valid = await db.prepare(`SELECT id FROM holds WHERE wall_id=? AND id IN (${placeholders})`).bind(wallId, ...uniqueIds).all()
    if ((valid.results ?? []).length !== uniqueIds.length) return apiError('INVALID_INPUT', 'One or more holds are invalid', 400)
  ...
  } catch (error) { console.error('problem_create_failed', error instanceof Error ? error.stack : String(error)); return apiError('INVALID_INPUT', 'Unable to create route', 400) }
}
const worker: ExportedHandler<Env> = { async fetch(request, env) {
  ...
  if (pathname.startsWith('/api/v1/')) {
    ...
    if (pathname === '/api/v1/problems' && request.method === 'POST') { const u = await session(request, env.DB); return u ? createProblem(request, env.DB, u) : apiError('AUTH_REQUIRED', 'Authentication required', 401) }
