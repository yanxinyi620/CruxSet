...
当前 Worker 已实现 `GET /api/v1/walls` 与 `GET /api/v1/problems`。两个接口都支持 `limit`（默认 20，最大 50）和 `cursor` 游标；线路接口还支持 `wallId` 筛选。没有绑定 D1 时接口会返回 `503 SERVICE_UNAVAILABLE`，不会回退到静态页面。
