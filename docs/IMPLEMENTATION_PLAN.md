...
- [-] 建立 `users`、`walls`、`layouts`、`problems`、`admins`、`counters` 集合（配置清单已文档化，真实环境待执行）。
...
- [-] 修复阻塞问题，生成微信体验版并完成 Phase 1 Freeze（已增加 `npm run verify:phase1` 结构门禁；真实 AppID、CloudBase 和真机仍待完成）。
