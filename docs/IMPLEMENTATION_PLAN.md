...
> **当前架构调整（2026-08-28）：** 本地 Web 创作工作台与小程序创作端分离；权威设计见 [本地 Web 创作工作台与小程序创作端设计](./superpowers/specs/2026-08-28-local-web-and-miniprogram-creation-design.md)，当前后续任务见 [双客户端后续实施计划](./superpowers/plans/2026-08-28-dual-client-next-steps.md)。本文件保留既有小程序 Phase 1 进度记录。
...
- [x] 新建线路可选择任一公开且至少有两个岩点的 Layout；未发布草稿只在“创建 → 我的草稿”二级页中继续标注，发布后永久锁定
- [x] “我的”仅显示用户墙面 / Layout 状态和删除管理；删除已发布 Layout 会级联删除其线路，删除墙面会级联删除所有 Layout 与线路
- [x] 三栏自定义 TabBar（线路 / 创建 / 我的），原生页面与浏览器预览采用相同的入口层级
...
- [-] 处理图片加载失败、空 Layout 和画布尺寸变化（加载失败提示、空 Layout 清屏已处理，尺寸变化待真机验证）。
...
- [x] 页面恢复时提示继续或丢弃草稿。
- [x] 网络失败保留草稿，服务端保存成功后删除草稿。
- [x] 在 UI 中解释三种 Foot Rule 的手脚权限。
...
- [-] 实现 `saveProblem` 服务端完整校验（入口、小程序调用服务与基础校验已建立，按逻辑 Layout ID 读取最高版本，编辑页已传递当前 Wall/Layout，待真实环境验证）。
- [-] 使用事务更新 `counters/problem_number`，生成唯一 `CS-000001` 编号（计数器读取、递增和线路写入已统一置于事务，待真实环境并发验证）。
...
- [-] 墙面列表和 Wall Detail 已有静态页面骨架（CloudBase 失败时保留 Demo 墙面并显示错误提示，墙面角度文案由页面层预先生成）。
- [-] 接入 Wall 与多个公开 Layout 数据（首页先选择 Wall，再选择具体公开 Layout；真实环境验证待完成）。
- [-] 实现角度、难度筛选和当前上下文持久化（Wall 页面已按 Wall ID 保存角度、难度和搜索词，云端数据验证待完成）。
...
- [-] 实现用户创建 Wall、上传图片与创建 Layout 流程（新建墙面后直接首次标注，草稿只可从“创建 → 我的草稿”二级页继续标注；发布后 Layout 公开且锁定，任一公开 Layout 都可定线，创建者可读取自己草稿图，真实 CloudBase 操作待验证）。
...
- [-] 修复阻塞问题，生成微信体验版并完成 Phase 1 Freeze（本地结构、页面入口、权限语义、不可变 Layout 发布流程、sitemap 与 `--release` AppID 门禁已建立；真实 AppID、CloudBase 和真机仍待完成）。
