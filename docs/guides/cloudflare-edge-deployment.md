...
跨平台发布申请需要 `0012_lab_publish_requests.sql`；已处理申请的删除功能还需应用 `0016_lab_publish_request_deletion.sql`，为申请增加共享删除标记。云端发布到小程序还需四项 CloudBase 服务端配置，详见[跨平台发布申请](segmentation-cloud.md#跨平台发布申请)。先应用迁移，再部署代码；申请和审核不需要修改 GitHub Actions 工作流。
