...
申请保存独立的图片和岩点快照，修改或删除原实验不影响审核。 删除已发布或已拒绝的申请时，实验台清理该申请的图片和岩点快照；历史回执保留，已发布的小程序墙面仍由小程序管理员管理。清理失败由现有定时任务重试。状态为待审核、发布中、已发布、已拒绝、发布失败；失败由管理员重试，重复提交及重试复用同一发布标识。已经开始过发布的申请不能再拒绝，避免目标已建墙但回执丢失时产生矛盾；云端中断的发布在 10 分钟处理期限结束后可以重试。
...
本地与云端实验台均支持管理员或申请者删除已发布、已拒绝的申请。操作栏使用与其他列表一致的红色“删除”按钮，确认后从双方列表移除；另一方已打开的页面在下一次自动刷新或手动刷新时更新。待审核、发布中及失败待恢复的申请不能删除。删除不影响已发布的墙面，后台保留防止重复发布所需的记录。墙面编号仅在鼠标悬浮于“已发布”状态时显示。
