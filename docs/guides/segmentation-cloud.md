...
当前云端模型入口提供 SAM 2.1（`sam2`），SAM 3 保留禁用占位；分块模型不再显示，历史分块任务仍可查看。SAM 2.1 使用 Ubuntu 24.04、Python 3.11、CPU PyTorch 和 4 个 CPU 线程。工作流超时为 90 分钟；Worker 中排队任务期限为 30 分钟，认领后的任务期限为 120 分钟。这些是不同层的超时保护。
