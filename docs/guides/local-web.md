...
本地和云端实验台共用按状态调整的刷新机制：运行中的分割任务及正在发布的申请每 5 秒刷新，待审核申请和管理员申请列表每 30 秒刷新，其余列表每 60 秒刷新。校准列表独立刷新，不随任务进度反复读取。上传、删除、提交等操作完成后立即刷新相关列表。

标签页隐藏时暂停自动请求，切回后刷新；操作弹窗期间暂缓自动刷新。内容未变化时保留现有列表。网络或服务错误按 10、20、40、80、120 秒退避，成功后恢复正常频率。登录失效或权限不足时停止自动请求，重新登录后可手动刷新或重新打开页面。
