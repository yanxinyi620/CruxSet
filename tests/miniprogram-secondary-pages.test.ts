...
test('detail and editor expose the Web-aligned visual structure', () => {
  ...
  expect(JSON.parse(readFileSync('wechat/miniprogram/pages/problem/detail/index.json','utf8')).navigationBarTitleText).toBe('线路详情'); expect(detail).toContain('{{problem.number}}'); expect(detail).toContain('legend-dot'); expect(detail).toContain('route-note')
