...
it('keeps mini program routes inside its package while preserving route behavior', () => {
  ...
  const draft = {
    id: 'problem_1', number: 'CS-000001', wallId: 'wall_1',
    angle: 35, grade: 'V4', holds: { start: ['H001'], finish: ['H002'] }, createdBy: 'usr_1', now: 100,
  }
  const wall = { id: 'wall_1', angleOptions: [35], holds: [{ id: 'H001' }, { id: 'H002' }] }
  expect(miniCreateProblem(draft as any, wall as any)).toEqual(createProblem(draft as any, wall as any))
})

it('matches web route filtering and searching behavior directly', () => {
  const wall = { id: 'wall_1', angleOptions: [25, 35], holds: [{ id: 'H001' }, { id: 'H002' }] }
  const drafts = [
    { id: 'problem_2', number: 'CS-000002', name: '左侧动态', wallId: 'wall_1', angle: 35, grade: 'V4', holds: { start: ['H001'], finish: ['H002'] }, createdBy: 'usr_1', now: 100 },
    { id: 'problem_1', number: 'CS-000001', name: '右侧静态', wallId: 'wall_1', angle: 25, grade: 'V3', holds: { start: ['H001'], finish: ['H002'] }, createdBy: 'usr_1', now: 100 },
  ]
  const webProblems = drafts.map(draft => createProblem(draft as any, wall as any))
  const miniProblems = drafts.map(draft => miniCreateProblem(draft as any, wall as any))
