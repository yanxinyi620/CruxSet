...
it.each(Array.from({length:15},(_,i)=>i*5))('accepts standard angle %s on a legacy wall',angle=>{expect(createProblem({...valid,angle},wall).angle).toBe(angle)})
