...
const wall:Wall={id:'wall_1',name:'日坛',description:'',activeLayoutId:'layout_1',angleOptions:[25,35],ownerId:'usr_1',visibility:'public',createdAt:1,updatedAt:1}
const layout:Layout={id:'layout_1',wallId:'wall_1',name:'2026-08',imageFileId:'cloud://wall',imageWidth:1000,imageHeight:800,geometryType:'circle',version:1,published:true,holds:[{id:'H001',x:.1,y:.1,radius:.02,kind:'hold'},{id:'H002',x:.2,y:.2,radius:.02,kind:'hold'},{id:'H003',x:.3,y:.3,radius:.02,kind:'hold'}],createdAt:1,updatedAt:1}
...
describe('problem validation',()=>{
  it('defaults to feet_follow, records layout version and keeps id distinct from number',()=>{const p=createProblem(valid,wall,layout);expect(p.footRule).toBe('feet_follow');expect(p.layoutVersion).toBe(layout.version);expect(p.id).not.toBe(p.number);expect(p.createdAt).toBe(100)})
