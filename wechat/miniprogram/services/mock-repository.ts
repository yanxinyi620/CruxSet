...
export class MockRepository {
  ...
  async createProblem(wallId:string,draft:Partial<Problem>){const wall=await this.getWall(wallId);if(wall.visibility!=='public'||wall.holds.length<2)throw new Error('WALL_NOT_ROUTABLE');const numbered=this.walls.filter(item=>Number.isInteger(item.wallNumber)&&item.wallNumber>0),missing=this.walls.filter(item=>!Number.isInteger(item.wallNumber)||item.wallNumber<=0).sort((a,b)=>a.createdAt-b.createdAt||a.id.localeCompare(b.id));let nextWallNumber=Math.max(0,...numbered.map(item=>item.wallNumber||0))+1;for(const item of missing)item.wallNumber=nextWallNumber++;const routeNumber=Math.max(0,...this.problems.filter(item=>item.wallId===wall.id).map(item=>Number(String(item.number||'').slice(-4))).filter(Number.isInteger))+1;const now=Date.now(),problem:Problem={id:`problem_mock_${now}`,number:`CS-${String(wall.wallNumber).padStart(2,'0')}${String(routeNumber).padStart(4,'0')}`,wallId,name:draft.name,description:draft.description,angle:draft.angle||wall.angleOptions[0],grade:draft.grade||'V0',footRule:draft.footRule||'feet_follow',holds:roles(draft.holds),createdBy:mockCurrentUserId,createdAt:now,updatedAt:now};this.problems.push(problem);return{id:problem.id,number:problem.number}}
