...
Page({data:{problemCount:0,isAdmin:false,error:''},onShow(){syncTabBar(this, 2);return this.reload()},async reload(){try{const [problems,isAdmin]=await Promise.all([listMyProblems(),currentUserIsAdmin()]);this.setData({problemCount:problems.length,isAdmin,error:''})}catch(error){this.setData({isAdmin:false,error:cloudErrorMessage(error)})}},openProfile(){wx.navigateTo({url:'/pages/profile/index'})},openWalls(){wx.navigateTo({url:'/pages/me/walls/index'})},openProblems(){wx.navigateTo({url:'/pages/me/problems/index'})},openManagement(){if(this.data.isAdmin)wx.navigateTo({url:'/pages/admin/management/index'})}})
