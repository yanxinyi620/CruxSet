...
import { currentUserId, ensureUser } from '../../../services/users.js'
...
Page({
  data: { wall: null, loading: true, problemId: '', problemNumber: '', editing: false, angles: routeAngles, grades, angle: 0, angleIndex: 0, grade: 'V4', gradeIndex: 4, roles, selectedRole: 'start', footRules, footRuleIndex: 0, footRule: 'feet_follow', footRuleLabel: labels.feet_follow, footRuleHint: hints.feet_follow, selected: {}, error: '', loadError: false, saving: false, saveDialogVisible: false, dialogName: '', dialogDescription: '' },
  async onLoad(options) { this.saved = false; draftKey = ''; try { await ensureUser(); problemId = options.problemId || options.id || ''; const loadedProblem = problemId ? await getProblem(problemId) : null; wallId = loadedProblem?.wallId || options.wallId || ''; draftKey = `problemDraft:${currentUserId()}:${problemId ? "edit:" + problemId : "new:" + wallId}`; const stored = wx.getStorageSync(draftKey) || {}; const saved = loadedProblem ? { ...loadedProblem, ...stored } : stored; editor = new ProblemEditor(saved.holds || {}); wall = await getWall(wallId); const angle = routeAngles.includes(saved.angle) ? saved.angle : 0, grade = saved.grade ?? 'V4', footRule = saved.footRule ?? 'feet_follow'; this.setData({ wall, loading: false, selectedRole: roles.some(role => role.id === saved.role) ? saved.role : 'start', problemId, problemNumber: saved.number || '', editing: Boolean(problemId), angles: routeAngles, selected: editor.value().holds, footRule, footRuleIndex: footRules.indexOf(footRule), footRuleLabel: labels[footRule], footRuleHint: hints[footRule], angle, angleIndex: routeAngles.indexOf(angle), grade, gradeIndex: grades.indexOf(grade), dialogName: saved.name || '', dialogDescription: saved.description || '' }) } catch (error) { this.setData({ loading: false, error: cloudErrorMessage(error), loadError: true }) } },
  onHide() { persist(this) },
  onUnload() { persist(this) },
  noop() {},
  selectRole(e) { this.setData({ selectedRole: e.currentTarget.dataset.role }); persist(this) },
