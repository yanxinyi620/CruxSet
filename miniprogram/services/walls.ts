...
export const createWall=(data:Partial<Wall>)=>isMockMode()?mockRepository.createWall(data):wallManager('createWall',data as Record<string,unknown>) as Promise<Wall>
export const updateWall=(id:string,patch:Partial<Wall>)=>isMockMode()?mockRepository.updateWall(id,patch):wallManager('updateWall',{id,...patch}) as Promise<Wall>
export const publishWall=(id:string)=>isMockMode()?mockRepository.publishWall(id):wallManager('publishWall',{id}) as Promise<Wall>
...
export const getWallImageUrl=(fileID:string)=>isMockMode()?mockRepository.getWallImageUrl(fileID):call<{url:string}>('getLayoutImageUrl',{fileID}).then(result=>result.url)
