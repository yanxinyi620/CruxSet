...
export const uploadWallImage = (filePath: string, cloudPath: string) => new Promise<{ fileID: string }>((resolve, reject) => { if (!wx.cloud) return reject(new Error('CLOUD_NOT_CONFIGURED')); wx.cloud.uploadFile({ cloudPath, filePath, success: resolve, fail: reject }) })
