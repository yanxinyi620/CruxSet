...
export class MockRepository {
  ...
  async deleteWall(id:string){const wall=await this.getWall(id);if(wall.ownerId!==mockCurrentUserId)throw new Error('FORBIDDEN');if(this.problems.some(p=>p.wallId===id))throw new Error('WALL_IN_USE');this.walls=this.walls.filter(w=>w.id!==id);return{ok:true as const}}
