...
export class MockRepository {
  ...
  private async writeLayout(wallId: string, layoutId: string, holds: Layout['holds'], publish: boolean) { const wall = await this.getWall(wallId); if (wall.ownerId !== mockCurrentUserId) throw new Error('FORBIDDEN'); const current = await this.getLayout(layoutId); if (current.wallId !== wallId) throw new Error('INVALID_LAYOUT_DATA'); if (current.published) throw new Error('LAYOUT_LOCKED'); const next: Layout = { ...current, holds: clone(holds), version: current.version + 1, published: publish, updatedAt: Date.now() }; this.layouts.push(next); if (publish) { const target = this.walls.find(item => item.id === wallId)!; target.visibility = 'public'; target.updatedAt = next.updatedAt } return clone(next) }
