...
export const getProblem = (id: string) => new Promise<Problem>((resolve, reject) => { if (!wx.cloud) return reject(new Error('CLOUD_NOT_CONFIGURED')); wx.cloud.database().collection('problems').doc(id).get({ success: result => resolve(result.data as Problem), fail: reject }) })
