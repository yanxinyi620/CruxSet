...
Component({ properties: { layout: { type: Object, value: null }, activeHolds: { type: Object, value: {} } }, data: { transform: { scale: 1, offsetX: 0, offsetY: 0 } }, methods: {
  onReady() { this.render() },
  render() { const query = wx.createSelectorQuery().in(this); query.select('#wallCanvas').fields({ node: true, size: true }).exec((result) => { const canvas = result[0]?.node; const layout = this.properties.layout as Layout | null; if (!canvas || !layout) return; const ctx = canvas.getContext('2d'); ctx.clearRect(0, 0, result[0].width, result[0].height); const active = this.properties.activeHolds as Record<string, string>; for (const hold of layout.holds) { const [x, y] = imageToScreen([hold.x * layout.imageWidth, hold.y * layout.imageHeight], this.data.transform); ctx.beginPath(); ctx.arc(x, y, Math.max(8, hold.radius * layout.imageWidth * this.data.transform.scale), 0, Math.PI * 2); ctx.fillStyle = active[hold.id] === 'start' ? '#39a96b' : active[hold.id] === 'foot' ? '#d7ad18' : active[hold.id] === 'finish' ? '#8b55c7' : active[hold.id] === 'assist' ? '#ef8f39' : hold.kind === 'volume' ? '#ef8f39' : '#316eea'; ctx.fill() } }) },
