...
Page({data:{walls:demo,loading:true,error:''},onLoad(){listWalls().then(walls=>this.setData({walls:walls.map(formatWall),error:''})).catch(error=>this.setData({error:error.message||'加载失败，请稍后重试'})).finally(()=>this.setData({loading:false}))},openWall(e){wx.navigateTo({url:`/pages/wall/index?id=${e.currentTarget.dataset.id}`})},openMine(){wx.navigateTo({url:'/pages/me/index'})}})
